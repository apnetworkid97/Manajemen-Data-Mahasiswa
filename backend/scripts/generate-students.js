...
function makeStudent(index, existingNims) {
  ...
  return {
    ...
    tanggalLahir: `${randomInt(2000, 2006)}-${String(randomInt(1, 12)).padStart(2, "0")}-${String(
      randomInt(1, 28)
    ).padStart(2, "0")}`,
...
async function run() {
  ...
  console.log(
    `Berhasil generate ${generated.length} data mahasiswa (mode: ${mode}). Total data sekarang: ${result.length}.`
  );
}
